...
### AC-P2-003: Withdraw（早期引き出し）

- [x] 満期前のポジション解消ができる（unwindPosition実装済み）
- [x] 予想受取額を表示できる
- [x] WithdrawActionコンポーネントで実際のトランザクション実行可能
- [x] 満期済みポジションはClaimに誘導

### AC-P2-003b: Claim（満期償還）

- [x] 満期済みポジションを自動検出できる
- [x] 元本・利息・合計受取額を表示できる
- [x] ClaimActionコンポーネントでexecuteRedemption実行可能
- [x] 「利息を受け取りたい」で満期償還フローが開始される

### AC-P2-004: x402決済

- [x] EIP-712署名データを生成できる
- [x] ガスレスで送金が完了する（Facilitator: http://localhost:4022）

### AC-P2-006: 購入フロー（住所ヒアリング）

- [x] 商品選択後にstartPurchaseで購入フローを開始できる
- [x] 住所情報（氏名・郵便番号・住所）をAIがヒアリングできる
- [x] confirmPurchaseで住所を確認し購入準備完了状態にできる
- [x] PurchaseActionコンポーネントで住所入力待ち/購入準備完了を区別して表示
- [x] 購入ボタンでx402署名→送金→注文作成ができる
